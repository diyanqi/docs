...
以 `_` 开头的 Class （内置 Class）有一些默认的基本设置。
...
再比如 `_File` Class 默认对所有人关闭了 `find` 权限，因为绝大多数应用可能不希望允许用户遍历文件表，获取所有文件。
不过为了兼顾开发测试阶段的便利性以及老应用的兼容性，默认设置比较宽松，老应用的内置 Class 的一些权限可能未作限制。
...
另外，**云服务控制台 > 内建账户 > 设置** 页面下有一些用户相关的安全选项，可以禁止未验证邮箱、手机的用户登录，要求修改密码时提供旧密码，密码修改后强制重新登录，验证第三方登录的 Access Token 是否有效。
