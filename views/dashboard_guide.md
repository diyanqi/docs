...
- 日志表不支持使用 LCQuery 查询，只能通过 [离线数据分析](datalake_guide.html) 功能查询分析。LeanCloud 云端会自动将日志表的数据按天导入离线分析平台。
- 日志表不支持 Class 绑定。
- 日志表不支持应用内搜索。
...
主要区域可切换显示数据、权限、性能与索引、Hooks、离线数据分析 五个标签页。
...
数据存储 API 对于结果集大小有限制，并且也不支持特别复杂、耗时较长的计算操作，对此，我们特别提供了「[离线数据分析](./datalake_guide.html)」这一功能，来支持产品和运营层面需要的数据统计／挖掘需求。
