...
基于现代的数据仓库 ClickHouse ，我们构建了新一代的数据湖分析服务。与其它云服务紧密集成，支持将存储数据，业务日志自动入库，从而为业务运营的统计分析提供强大的数据支持。

相比传统的 Spark/HDFS 架构的离线分析，新的数据湖分析将提供以下独特的功能，
...
在控制台「数据存储」页面，创建日志表即可开启日志表的数据分析功能。日志会随写入实时入库，并且实时可查。

### Class 对象数据入库

在控制台「数据仓库」页面，点击「创建 class 同步」，选择需要查询分析的字段，即可启用 Class 到数据仓库的同步。存量数据会异步进行同步，取决于数据量的大小，
同步过程可能有较大延迟，请耐心等待。

Class 同步启用后，我们会在次日凌晨同步前一日更新过的数据。因此更新数据需要在次日才可见。
...
注意字符串，日期等值必需用**单引号**引用；而对于字段或表名中有特殊字符的情况，可使用反引号进行引用，
比如
...
更多查询语法请参考 ClickHouse 文档指南。基于性能考量，我们会限制部分语法。
...
在数据同步的过程中，多层嵌套的 object 对象会以 json 字符串的形式入库。为了提取 json 字符串中的字段，推荐使用
`visitParamExtract*` 来提取字段，比如

```sql
SELECT visitParamExtractString('{"abc":"\\u263a"}', 'abc') = '☺';
```

在嵌套复杂，且出现有重复字段名称时，可使用 `JSONExtract*`，比如
...
在按照日期进行统计分析时，可以使用 `toYYYYMMDD` 将日期字符串化后进行 group by 统计。
同时需要注意时区的处理，比如，
